...
impl DiffLine {
    ...
    pub fn new(
        ...
        Self {
            ...
            should_highlight: matches!(change_type, LineChangeType::Unchanged | LineChangeType::Modified | LineChangeType::Added),
            prefix,
        }
    }
    ...
    pub fn from_unified(line: &str, line_num: usize) -> Option<Self> {
        ...
        Some(Self::new(left_line, right_line, content, change_type, prefix))
    }
            ...
            if let Some(diff_line) = DiffLine::from_unified(line, i + 1) {
                match diff_line.change_type {
                    ...
                    LineChangeType::FileHeader => {
                        ...
                        if line.starts_with("--- ") {
                            old_file_path = Some(line[4..].trim().to_string());
                        } else if line.starts_with("+++ ") {
                            new_file_path = Some(line[4..].trim().to_string());
                        }
                    }
                    _ => {}
                }
