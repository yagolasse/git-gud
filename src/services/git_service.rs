...
use git2::{
    BranchType, Commit, DiffOptions, ErrorCode, Repository, Status, StatusOptions,
};
...
impl GitService {
    ...
    pub fn get_status(repo: &Repository) -> Result<(Vec<models::FileChange>, Vec<models::FileChange>)> {
        ...
        log::debug!("Found {} unstaged files, {} staged files", 
                   unstaged_files.len(), staged_files.len());
    ...
    pub fn get_branches(repo: &Repository) -> Result<Vec<models::Branch>> {
        ...
        let current_branch = repo.head().ok().and_then(|r| r.shorthand().map(|s| s.to_string()));
        ...
        for branch_result in local_branches {
            ...
            let commit_id = branch.get().peel_to_commit()
