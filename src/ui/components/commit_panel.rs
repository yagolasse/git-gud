...
impl CommitPanel {
    ...
    pub fn show(&mut self, ui: &mut egui::Ui, state: &mut AppState) {
        ...
        // Character counter
        let total_chars = state.ui_state.commit_summary.len() + state.ui_state.commit_description.len();
        ui.horizontal(|ui| {
        ...
        ui.collapsing("Advanced options", |ui| {
            ...
            ui.checkbox(&mut state.ui_state.show_only_staged, "Show only staged files");
            ui.checkbox(&mut state.ui_state.show_only_unstaged, "Show only unstaged files");
        });
        ...
        ui.horizontal(|ui| {
            ...
            if ui.add_enabled(commit_enabled, egui::Button::new("💾 Commit"))
    ...
    fn create_commit(&mut self, state: &mut AppState) {
        ...
        match state.repository_state_mut().create_commit(&message) {
            Ok(()) => {
                state.set_info(format!("Commit created: {}", 
