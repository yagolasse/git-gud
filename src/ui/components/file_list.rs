...
impl FileList {
            ...
            if file_count > 0 {
                if self.is_staged {
                    if ui.button("Unstage All").clicked() {
                        ...
                        state.ui_state.pending_action = Some(crate::state::PendingAction::UnstageAll(all_paths));
                    }
                } else {
                    if ui.button("Stage All").clicked() {
                        let all_paths: Vec<_> = files.iter().map(|f| f.path.clone()).collect();
                        // We need to modify state, so we'll handle this after the UI rendering
                        state.ui_state.pending_action = Some(crate::state::PendingAction::StageAll(all_paths));
                    }
                }
            }
        });
            ...
            ui.horizontal(|ui| {
                ...
                if self.is_staged {
                    if ui.button("Unstage Selected").clicked() {
                        ...
                        state.ui_state.pending_action = Some(crate::state::PendingAction::UnstageSelected(selected_paths));
                    }
                } else {
                    if ui.button("Stage Selected").clicked() {
                        let selected_paths: Vec<_> = self.checked_files.iter().cloned().collect();
                        state.ui_state.pending_action = Some(crate::state::PendingAction::StageSelected(selected_paths));
                    }
                }
            });
        }
    ...
    fn show_file(&mut self, ui: &mut egui::Ui, state: &mut AppState, file: &crate::models::FileChange) {
        ui.horizontal(|ui| {
            ...
            response.context_menu(|ui| {
                if self.is_staged {
                    if ui.button("Unstage").clicked() {
                        if let Err(e) = state.repository_state_mut().unstage_files(&[file.path.clone()]) {
                    ...
                    if ui.button("Stage").clicked() {
                        if let Err(e) = state.repository_state_mut().stage_files(&[file.path.clone()]) {
