...
impl AppState {
    ...
    pub fn load_repository(&mut self, path: PathBuf) -> anyhow::Result<()> {
        ...
        self.ui_state.selected_branch = self.repository_state()
            .branches
            .iter()
            .find(|b| b.is_current)
            .map(|b| b.name.clone());
